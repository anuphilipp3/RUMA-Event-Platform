-- FIX: two staff roles.
--   'admin'   = you: full dashboard (approve, reports, lucky draw, events)
--   'scanner' = counter volunteers: ONLY the check-in page, nothing else
-- Paste into the Supabase SQL Editor and Run. Safe to re-run.

-- Ensure the lucky-draw switch exists too (so this file works standalone).
alter table public.events
  add column if not exists lucky_draw_enabled boolean not null default false;

alter table public.admins
  add column if not exists role text not null default 'admin'
  check (role in ('admin', 'scanner'));

-- Full-organizer check (admins only, not scanners).
...
-- Privileged actions become organizer-only (scanners can't approve/draw).
...
-- Direct table access (defense-in-depth) → organizer-only. Scanners reach the
-- check-in flow only through the SECURITY DEFINER check_in_ticket function.
do $$
declare t text;
begin
